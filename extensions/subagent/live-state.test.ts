...
function message(timestamp: number, text: string, responseId?: string) {
	return { role: "assistant", timestamp, api: "test-api", provider: "p", model: "m", responseId, content: [{ type: "text", text }] };
}
...
test("assistant snapshots replace within a generation and a shorter new turn replaces an older one", () => {
	...
	assert.equal(updateAssistantMessage(live, message(1, "first long response"), 1024), true);
	assert.equal(live.currentAssistantText, "first long response");
	assert.equal(finalizeAssistantMessage(live, message(1, "first long response"), 1024), true);
...
test("finalization tombstones reject delayed same-message events and duplicate finalization", () => {
	...
	assert.equal(updateAssistantMessage(live, message(1, "stale partial"), 1024), false);
	assert.equal(startAssistantMessage(live, message(1, "")), false);
	assert.equal(finalizeAssistantMessage(live, message(1, "duplicate final"), 1024), false);
...
test("unseen responseIds distinguish legitimate same-timestamp messages", () => {
	...
	assert.equal(finalizeAssistantMessage(live, message(4, "first", "response-a"), 1024), true);
	assert.equal(startAssistantMessage(live, message(4, "", "response-b")), true);
	assert.equal(finalizeAssistantMessage(live, message(4, "short", "response-b"), 1024), true);
	assert.equal(startAssistantMessage(live, message(4, "", "response-c")), true);
	assert.equal(finalizeAssistantMessage(live, message(4, "new", "response-c"), 1024), true);
...
test("a fallback identity can upgrade to a documented provider responseId", () => {
	...
	assert.equal(updateAssistantMessage(live, message(6, "partial", "response-late"), 1024), true);
	assert.equal(finalizeAssistantMessage(live, message(6, "final", "response-late"), 1024), true);
	...
	assert.equal(startAssistantMessage(live, message(6, "", "response-next")), true);
	assert.equal(finalizeAssistantMessage(live, message(6, "next", "response-next"), 1024), true);
	assert.equal(startAssistantMessage(live, message(6, "")), false);
	assert.equal(startAssistantMessage(live, message(6, "", "response-late")), false);
});
...
test("retained older finalized identities reject start, update, and end records", () => {
	...
	assert.equal(startAssistantMessage(live, message(7, "", "response-new")), true);
	assert.equal(finalizeAssistantMessage(live, message(7, "new", "response-new"), 1024), true);

	assert.equal(startAssistantMessage(live, { ...first, content: [] }), false);
	assert.equal(startAssistantMessage(live, message(7, "", "response-current")), true);
	...
	assert.equal(live.resultText, "new");
	assert.equal(finalizeAssistantMessage(live, message(7, "current", "response-current"), 1024), true);
	assert.equal(live.currentAssistantText, "current");
...
test("finalized assistant identity history is FIFO-bounded to 256 entries", () => {
	...
	for (let index = 0; index <= MAX_FINALIZED_ASSISTANT_IDENTITIES; index++) {
		...
		assert.equal(finalizeAssistantMessage(live, message(9, responseId, responseId), 1024), true);
	}

	assert.equal(MAX_FINALIZED_ASSISTANT_IDENTITIES, 256);
	assert.equal(live.finalizedAssistantIdentities.length, MAX_FINALIZED_ASSISTANT_IDENTITIES);
	assert.equal(live.finalizedAssistantIdentities[0]?.responseId, "response-1");
	assert.equal(live.finalizedAssistantIdentities.at(-1)?.responseId, "response-256");
	assert.equal(startAssistantMessage(live, message(9, "", "response-1")), false);
...
test("late snapshots from an older assistant message are ignored", () => {
	...
	assert.equal(updateAssistantMessage(live, message(1, "late old snapshot"), 1024), false);
	assert.equal(live.currentAssistantText, "new");
});

test("parallel tool output stays correlated and bounded", () => {
	const first: ToolActivity = { toolCallId: "a", name: "bash", startedAt: 1, updatedAt: 1, output: "", outputTruncated: false };
	const second: ToolActivity = { toolCallId: "b", name: "read", startedAt: 2, updatedAt: 2, output: "", outputTruncated: false };
	updateToolActivity(first, { content: [{ type: "text", text: `prefix-${"x".repeat(50)}-tail-a` }], details: { progress: 25 } }, 16, 3);
	updateToolActivity(second, { content: [{ type: "text", text: "output-b" }] }, 16, 4);
