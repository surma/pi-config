...
sources=(child.ts dispatch-event.ts index.ts lifecycle.ts live-state.ts lock.ts owner.ts registry.ts result-store.ts settlement-notifications.ts ui.ts)
tests=(guard.test.ts lifecycle.test.ts liveness.test.ts live-state.test.ts lock.test.ts dispatch-event.test.ts launch.test.ts owner.test.ts registry.test.ts result-store.test.ts settlement-notifications.test.ts tools.test.ts ui.test.ts)
