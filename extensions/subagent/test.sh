...
sources=(child.ts dispatch-event.ts index.ts ipc.ts ipc-child.ts lifecycle.ts live-state.ts owner.ts registry.ts result-store.ts settlement-notifications.ts ui.ts zellij.ts)
tests=(guard.test.ts ipc.test.ts ipc-child.test.ts child-bridge.test.ts lifecycle.test.ts live-state.test.ts dispatch-event.test.ts launch.test.ts owner.test.ts registry.test.ts result-store.test.ts settlement-notifications.test.ts tools.test.ts ui.test.ts)
