...
export default function (pi: ExtensionAPI) {
	...
	// Cache rendered lines for finished (immutable) thread answers, keyed by text.
	// A fresh Markdown instance per call defeats the SDK's per-instance cache, so
	// without this every thread item is re-parsed on every render frame.
	const markdownLineCache = new Map<string, { width: number; lines: string[] }>();

	function renderCachedMarkdownLines(text: string, width: number): string[] {
		const cached = markdownLineCache.get(text);
		if (cached && cached.width === width) return cached.lines;
		const lines = renderMarkdownLines(text, width);
		markdownLineCache.set(text, { width, lines });
		return lines;
	}
	...
	function getTranscriptLinesInner(width: number, theme: ExtensionContext["ui"]["theme"]): string[] {
		...
		for (const item of thread) {
			...
			// Assistant message rendered as markdown (cached — thread answers are immutable)
			const mdLines = renderCachedMarkdownLines(item.answer, width);
			lines.push(...mdLines);
			lines.push("");
		}
